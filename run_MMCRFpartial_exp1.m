...
function rtn = run_MMCRFpartial_exp1(inname,P_missingIn)

P_missingIn=eval(P_missingIn);
...
for i=1:size(Elist,1)
    ...
    for k=1%nfold
        ...
        perfPer(j,:)=[per,P_missing,acc,vecacc,pre,rec,f1,auc1,auc2]
        


        % save results
        dlmwrite(sprintf('../results/MMPartial_%s',paramsIn.filestem),perfPer)

    end
end

system(sprintf('rm %s.log Ypred_%s.mat',paramsIn.filestem,paramsIn.filestem));
